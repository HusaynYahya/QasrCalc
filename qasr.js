...
(function () {
  ...
  function cityTooBig(city) {
    /* A hand-drawn ring and an official built-up area are both deliberate
       choices of line. Warning about a border the page went and fetched on
       purpose tells the reader nothing they can act on. */
    if (!city || !city.shape || city.fromRing || city.fromUrban) return false;
  ...
  /* Dubai, bounded inland by the E611 and seaward by the coast.

     The boundary published for Dubai is the emirate: 5,970 km2 reaching
     twelve kilometres into the Gulf, because a legal boundary includes
     territorial water, and ninety-seven kilometres inland to Hatta. Neither
     is the city, and drawing it was simply wrong.

     The city is what the E611 encloses against the sea, which is the line
     Google draws and the one a resident describes. Built by cutting the
     published boundary along the E611's own centreline, traced from
     OpenStreetMap, and then keeping only the land — so the coast is the real
     coast, and Palm Jumeirah, the Marina and the other reclaimed ground stay
     in Dubai, which they are. Fifteen parts for that reason: the islands are
     their own.

     Land from OpenStreetMap's coastline (osmdata.openstreetmap.de,
     simplified land polygons, ODbL); the E611 from Overpass. Checked against
     twenty-three places either side of it — Downtown, Deira, the Marina, the
     Palm, Al Barsha, Silicon Oasis, Mirdif, Arabian Ranches, Motor City,
     Sports City, International City, Jebel Ali, Al Qusais, Investment Park
     and the airport inside; Al Lisaili, Margham, Hatta, three points out to
     sea, Sharjah and Abu Dhabi outside.                                     */
  var DUBAI = [
    [[
      [55.04427, 24.83957],
      [55.02374, 24.86817],
  ...
  var RING_ROADS = [
    ...
    { city: "Dubai", area: "United Arab Emirates", edge: "E611 and the coast",
      box: [24.79, 54.88, 25.37, 55.57],
      shape: { type: "MultiPolygon", coordinates: DUBAI },
      areaKm2: 1328 }
  ];
  ...
  /* The official built-up area, taken in place of whatever the address
     service gave. regionKm2 is the size of what it replaced, left unset where
  ...
  function takeUrbanArea(city, urban) {
    ...
    city.urbanSource = urban.source || null;
    return city;
  }
  ...
  function cityWithRing(place, announce) {
    ...
    return cityOf(place).then(function (city) {
      return urbanAreaAt(place).then(function (urban) {
        /* Where the country's own statistics office publishes the edge of the
           built-up area, that is the border — before any question of whether
           OpenStreetMap's is too big, too small or missing.

           The two are not rival guesses at one line. OpenStreetMap publishes
           administrative boundaries, drawn for administrative reasons; the
           statistics offices publish where the houses stop, from building
           density on a 25 m grid. The ruling counts from the end of the city
           [1704], which is the second of those, and the difference is not
           small: Leeds the built-up area is 119 km2, Leeds the metropolitan
           district 552, with farmland and three market towns in between. The
           sweep called that one "nothing obviously wrong", because 552 is
           under the size a region is caught at.

           Only sources that publish town by town are in the file. See
           tools/references.py for what is held back and why. */
        if (urban) return takeUrbanArea(city, urban);
        ...
        /* Nothing official, and whatever the address service gave. Too large
           or missing, showCity says so and changes nothing. */
        return city;
      });
    });
  }
    ...
    if (city && city.name) {
      hint.innerHTML = lead + " <b>" + city.name + "</b>" +
        ...
        (slot !== "from" ? ""
          ...
          /* Said with its source named, because it is the strongest claim
             the page makes about a border it did not draw by hand, and the
             reader should be able to go and check it. */
          : city.fromUrban ? " — its built-up area as published by " +
              (city.urbanSource || "the national statistics office") + ", " +
              city.urbanKm2.toLocaleString("en-GB") + " km², is outlined on the map: " +
              "where the houses stop, which is what the ruling counts from" +
              (city.regionKm2
                ? ", rather than the " + city.regionKm2.toLocaleString("en-GB") +
                  " km² administrative boundary published under the name."
                : ".")
